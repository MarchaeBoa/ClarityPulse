...
const plans = [
  {
    ...
    yearlyPrice: 0,
    cta: "Comece grátis",
    ctaStyle: "border border-white/[0.08] text-ghost hover:text-white hover:border-white/[0.15]",
    popular: false,
    features: [
  ...
  },
  {
    ...
    ctaStyle: "bg-jade text-ink hover:bg-jade-hover jade-glow-subtle",
    popular: true,
    features: [
    ...
    <section id="pricing" className="relative py-24 lg:py-32">
      ...
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <div className="flex items-center justify-center gap-4 mb-12">
          <span className={`text-[13px] ${!annual ? "text-white" : "text-ghost"} transition-colors`}>
            Mensal
          </span>
          <button
          ...
          >
            <div
              className={`absolute top-0.5 w-5 h-5 rounded-full bg-jade transition-all duration-300 ${
            ...
            return (
              <div
                ...
                className={`relative flex flex-col p-6 rounded-2xl border transition-all duration-300 ${
                  plan.popular
                    ? "bg-white/[0.03] border-jade/20 shadow-xl"
                    : "bg-white/[0.01] border-white/[0.06] hover:border-white/[0.1]"
                }`}
              >
                {plan.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span className="px-3 py-1 rounded-full bg-jade text-ink text-[10px] font-mono font-medium uppercase tracking-wider">
                      Mais popular
                    </span>
                  </div>
                )}
                ...
                <a
                  href="#"
                  className={`block text-center px-5 py-3 rounded-xl font-display font-semibold text-[13px] transition-all duration-200 active:scale-[0.97] mb-6 ${plan.ctaStyle}`}
                >
                  {plan.cta}
                </a>
