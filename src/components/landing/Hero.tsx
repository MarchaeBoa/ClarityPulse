...
function DashboardMockup() {
  return (
    <div className="relative w-full">
      ...
      >
        <div className="bg-surface rounded-xl border border-white/[0.06] overflow-hidden shadow-2xl shadow-black/40">
          {/* Topbar */}
          ...
          <div className="grid grid-cols-4 gap-px bg-white/[0.03] p-4">
            {[
              { label: "VISITANTES", value: "24,847", change: "+12.4%", up: true },
              { label: "PAGEVIEWS", value: "68,432", change: "+8.2%", up: true },
              { label: "BOUNCE RATE", value: "34%", change: "-3.1%", up: true },
              { label: "TEMPO MÉDIO", value: "4m07s", change: "+18s", up: true },
            ].map((kpi) => (
              <div key={kpi.label} className="bg-surface-2 rounded-lg p-3">
                <span className="text-[9px] font-mono font-medium tracking-wider text-ghost/60 uppercase block mb-2">
                  {kpi.label}
                </span>
      ...
      <Float delay={0} duration={6} y={14}>
        <div className="absolute -right-2 top-16 bg-surface border border-white/[0.08] rounded-xl p-3 shadow-xl shadow-black/20 hidden lg:block">
      ...
      <Float delay={2} duration={7} y={10}>
        <div className="absolute -left-2 bottom-20 bg-surface border border-white/[0.08] rounded-xl p-3 shadow-xl shadow-black/20 hidden lg:block">
          <div className="flex items-center gap-2">
...
export default function Hero() {
  ...
  return (
    <section ref={containerRef} className="relative min-h-screen flex items-center overflow-hidden">
      {/* Background effects */}
      <motion.div className="absolute inset-0 hero-gradient" style={{ opacity: bgOpacity }} />
        ...
        <div className="grid lg:grid-cols-[45%_55%] gap-12 lg:gap-10 items-center">
          ...
          <div>
            ...
            <motion.h1
              ...
              className="font-display font-extrabold text-[clamp(36px,5.5vw,64px)] leading-[0.92] tracking-[-0.03em] text-white mb-6"
            >
              Veja o que importa,{" "}
              <span className="text-jade inline-block">não o que é permitido.</span>
            </motion.h1>
            ...
            >
              <motion.a
                href="#pricing"
                whileHover={{ scale: 1.03, boxShadow: "0 0 40px rgba(26,229,160,0.2)" }}
                whileTap={{ scale: 0.97 }}
                className="inline-flex items-center gap-2.5 px-6 py-3 rounded-xl bg-jade text-ink font-display font-semibold text-[14px] hover:bg-jade-hover transition-colors duration-200 jade-glow-subtle"
              >
              ...
              <motion.a
                ...
                whileTap={{ scale: 0.97 }}
                className="inline-flex items-center gap-2 px-5 py-3 rounded-xl border border-white/[0.08] text-ghost hover:text-white font-medium text-[14px] transition-all duration-200"
              >
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            ...
            >
              {["Script de 4.8kb", "Setup em 2 minutos", "Sem cartão de crédito"].map((signal) => (
                <div key={signal} className="flex items-center gap-2">
                  <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                    <path d="M2.5 6L5 8.5L9.5 4" stroke="#1AE5A0" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  <span className="text-[11px] font-mono text-ghost/50 tracking-wide">{signal}</span>
                </div>
              ))}
            </motion.div>
          </div>
