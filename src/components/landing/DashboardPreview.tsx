        ...
        <motion.div className="relative" style={{ scale, opacity }}>
          ...
          <div className="relative bg-surface rounded-2xl border border-white/[0.06] overflow-hidden shadow-2xl shadow-black/40">
            ...
            <div className="flex min-h-[500px]">
              {/* Sidebar */}
              <div className="hidden lg:flex flex-col w-[200px] bg-[#111214] border-r border-white/[0.05] p-4">
                {/* Logo */}
                <div className="flex items-center gap-2 mb-6 pb-4 border-b border-white/[0.05]">
                  ...
                  <span className="text-[12px] font-display font-semibold text-white">ClarityPulse</span>
                </div>
                ...
                <div className="flex items-center gap-2 p-2 rounded-lg bg-white/[0.03] border border-white/[0.05] mb-5">
                  <div className="w-5 h-5 rounded bg-sapphire/10 flex items-center justify-center">
                    <span className="text-[8px] font-mono font-bold text-sapphire">M</span>
                  </div>
                  <span className="text-[11px] text-ghost truncate flex-1">meusite.com.br</span>
                  <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                    <path d="M3 4L5 6L7 4" stroke="#5A5E6B" strokeWidth="1.2" strokeLinecap="round"/>
                  </svg>
                </div>
                ...
                <nav className="flex flex-col gap-0.5">
                  {[
                    ...
                    { label: "Páginas", active: false },
                    { label: "Fontes", active: false },
                    { label: "Geo & Devices", active: false },
                    { label: "Conversões", active: false },
              ...
              <div className="flex-1">
                {/* Topbar */}
                <div className="flex items-center justify-between px-5 py-3 h-[48px] border-b border-white/[0.05]">
                  <h3 className="text-[13px] font-display font-semibold text-white">Dashboard</h3>
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-white/[0.03] border border-white/[0.06]">
                      <span className="text-[10px] font-mono text-ghost">Últimos 7 dias</span>
                      <svg width="8" height="8" viewBox="0 0 8 8" fill="none">
                        <path d="M2 3L4 5L6 3" stroke="#5A5E6B" strokeWidth="1.2" strokeLinecap="round"/>
                      </svg>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className="w-1.5 h-1.5 rounded-full bg-jade animate-pulse" />
                      <span className="text-[10px] font-mono text-ghost/50">Live</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 p-4">
                  {[
                    { label: "VISITANTES ÚNICOS", value: "24,847", change: "+12.4%", up: true },
                    { label: "PAGEVIEWS", value: "68,432", change: "+8.2%", up: true },
                    { label: "BOUNCE RATE", value: "34.2%", change: "-3.1%", up: true },
                    { label: "DURAÇÃO MÉDIA", value: "4m 07s", change: "+18s", up: true },
                  ].map((kpi) => (
                    <div key={kpi.label} className="bg-surface-2 rounded-xl p-4 border border-white/[0.03]">
                      <span className="text-[9px] font-mono font-medium text-ghost/50 uppercase tracking-wider block mb-2">
                        {kpi.label}
                      </span>
                      <div className="flex items-end gap-2">
                        ...
                        <span className={`text-[10px] font-mono mb-0.5 ${kpi.up ? "text-jade" : "text-ember"}`}>
                          {kpi.change}
                        </span>
                      </div>
                ...
                <div className="grid lg:grid-cols-[1fr_280px] gap-3 px-4 pb-4">
                  <div className="bg-surface-2 rounded-xl p-4 border border-white/[0.03]">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex gap-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[9px] font-mono text-ghost/50">Atual</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <div className="w-2 h-1 rounded-full bg-white/10" />
                          <span className="text-[9px] font-mono text-ghost/50">Anterior</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between mt-3">
                      {["Seg", "Ter", "Qua", "Qui", "Sex", "Sáb", "Dom"].map((d) => (
                        <span key={d} className="text-[9px] font-mono text-ghost/30">{d}</span>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Top Sources */}
                  <div className="bg-surface-2 rounded-xl p-4 border border-white/[0.03]">
                    <span className="text-[10px] font-mono text-ghost/50 uppercase tracking-wider block mb-4">
                      Top Fontes
                    </span>
                      ...
                      {[
                        ...
                        <div key={s.source}>
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="text-[11px] text-ghost">{s.source}</span>
            ...
            {[
              {
                ...
                description: "Veja visitantes navegando pelo seu site agora. Sem delay de 24-48h como no GA4.",
              },
              {
                title: "Comparação de períodos",
                description: "Compare qualquer intervalo com o período anterior. Tendências ficam óbvias.",
              },
              {
